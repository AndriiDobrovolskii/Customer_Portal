...
async def test_register_valid_input_returns_201_with_location_and_body(
    ...
    assert response.headers["location"] == f"/v1/users/{body['id']}"
...
async def test_concurrent_duplicate_registration_only_one_succeeds(
    ...
    responses = await asyncio.gather(
        real_client.post(
            "/v1/auth/register", json={"email": email_lower, "password": "Str0ng!Pass1"}
        ),
        real_client.post(
            "/v1/auth/register", json={"email": email_upper, "password": "Str0ng!Pass1"}
        ),
    )
