...
description: Breaks an approved plan and impact analysis into small, ordered, independently-verifiable implementation tasks — which execution skill to invoke and in what sequence. For a `track: backend` Story that's schema-builder, data-layer-builder, migration-manager, service-and-router-builder, respecting AGENTS.md §3 layering (models/migration before repository, repository before service, service before router) and this project's "migration before model use" rule; for a `track: frontend` Story that's frontend-builder, respecting AGENTS.md §3's Frontend layer table (api-client and store before the hooks/screens that depend on them). Use when a plan and impact analysis are approved and work needs to be sequenced into discrete steps ("break this into tasks," "what order do I build this in," "task list for US-xxx"). Similar in spirit to agent-skills:planning-and-task-breakdown but scoped to this project's execution skills and layering order; it does not decide what changes (that's planner/impact-analyzer) and does not itself write schema/model/service/router/frontend code or run the gate (that's the execution skills and gate-enforcer).
---
...
absent means `backend`) before sequencing anything — it picks which execution-skill set and
which ordering rule below applies.
...
3. `docs/designs/database/...` and `docs/designs/api/...` — to attribute each task to the right layer/skill (`track: backend` only; `NOT_APPLICABLE`/absent for `track: frontend`, which attributes tasks against `AGENTS.md` §3's Frontend layer table instead).
4. `AGENTS.md` §3 — the layering direction this skill's ordering rule is derived from (the base table for backend, its Frontend subsection for frontend).
...
1. For every affected file/layer named in the impact analysis, emit one task naming:
   - the execution skill (`frontend-builder` — the only one on this track today),
   - its `AGENTS.md` §3 Frontend layer (`api/`, `store/`, `hooks/`, `routes/`, `screens/`),
   - its depends-on tasks (by Task ID),
   - a concrete verification step (e.g. "vitest passes for this hook," "tsc --noEmit clean," "grep confirms zero fetch imports in this screen").
2. **Ordering rule, derived from AGENTS.md §3's Frontend layer table:** the scaffold (if `frontend/` doesn't exist yet) before everything; `api/` and `store/` before `hooks/` (a hook needs both to exist); `hooks/` before `routes/` and `screens/` (a route guard or screen needs the hooks it calls); within one screen, its form/validation task before the screen task that wires it in. `api/` and `store/` may run in parallel (neither depends on the other).
...
- [ ] The sequence respects §3's downward-only import direction (backend: the migration-before-model-use rule; frontend: api/store before hooks before routes/screens).
