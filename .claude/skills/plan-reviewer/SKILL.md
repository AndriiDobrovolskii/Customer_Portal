...
Both `docs/plans/<StoryId>-implementation-plan.md` and `docs/plans/<StoryId>-task-breakdown.md` must exist, alongside the spec and impact analysis they derive from. For a `track: backend` Story (`docs/stories/<StoryId>.md` front matter; absent means backend), the API/DB designs must exist too, unless recorded `NOT_APPLICABLE`. For `track: frontend`, API/DB designs are `NOT_APPLICABLE` by definition — do not wait for them. If a genuinely required input is missing, say so and stop rather than reviewing an incomplete set.

## Step-by-Step Review Workflow

1. **Extract ground truth.** Read the spec and impact analysis first, plus the API/DB designs for a `track: backend` Story (`AGENTS.md` §3's Frontend subsection instead, for `track: frontend`) — these are what the plan and task breakdown must be measured against, not each other.
...
4. **Check layering order in the task breakdown.** For `track: backend`, `implementation-planner`'s sequence must respect `AGENTS.md` §3's downward-only direction and this project's migration-before-model-use rule. For `track: frontend`, it must respect §3's Frontend subsection: scaffold before everything, `api/`/`store/` before `hooks/`, `hooks/` before `routes/`/`screens/`. Either way, flag any task that depends on a layer built after it.
5. **Check risk realism.** For `track: backend`, does `planner`'s Risks section cover migration hazards (`AGENTS.md` §4 "Migrations"), concurrency, and contract-breaking changes where the impact analysis or DB design implies they're relevant? For `track: frontend`, does it cover the concurrent-401/single-in-flight-refresh race and any other Open Decision the impact analysis flagged as blast-radius-altering — not just a generic "testing will catch issues" placeholder either way?
6. **Check test-strategy realism** against `AGENTS.md` §5's split for the Story's track — unit-fake vs. integration-on-real-infrastructure (backend), or Vitest-unit vs. RTL-plus-MSW-integration (frontend) — does the plan's Testing Strategy actually name which parts are which, or is it vague?
