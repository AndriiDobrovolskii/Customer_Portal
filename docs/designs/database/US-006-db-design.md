...
**Spec:** `docs/specifications/US-006-logout-spec.md` (Pass with Issues, accepted 2026-08-31)
**API:** `docs/designs/api/US-006-openapi.yaml`, `US-006-api-design.md`
...
Rejected alternative: reusing the existing `reason` column for `scope`'s values. Not used — `reason`'s established meaning across every other row in this table is "why did the attempt fail" (`bad_password`, `unknown_email`, etc.); overloading it with a "which blast radius, on a *successful* logout" concept would make the column's semantics ambiguous for anyone reading the table without first checking `event`. A dedicated column keeps each field single-purpose, matching the project's existing audit-table style (one column, one meaning, per US-005's own `auth_audit_log` design).

### Indexes

- `ix_refresh_tokens_family_id` on `refresh_tokens(family_id)` — new, per above.
- No new index on `auth_audit_log.scope`: no FR or NFR in this story states a query pattern filtering by `scope` alone; the existing `actor_id` index already serves the anticipated "audit history for this account" pattern (US-3.3's eventual audit-view query is not this story's concern to pre-guess, same reasoning US-005's design applied).
