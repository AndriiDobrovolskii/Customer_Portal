...
- **`ix_refresh_tokens_user_id_family_id_issued_at`** *(new)* on `refresh_tokens(user_id, family_id, issued_at)` — supports both aggregate query shapes above as an ordered/index-only scan. See `US-010-db-design.md` for the p95-budget rationale.
- No new index on `auth_audit_log.target_family`: this story only writes it, per `US-010-db-design.md`'s Indexes section.
...
- A first-class `refresh_token_families` table — considered and rejected, see `US-010-db-design.md`.
