...
**Indexes:** no new index. The existing `ix_auth_audit_log_actor_id` on `actor_id` already serves the anticipated "audit history for this account" query pattern (per US-005's own precedent); no FR or NFR in this story states a query pattern filtering by `scope` alone.
