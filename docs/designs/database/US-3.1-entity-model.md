# Entity Model: Manage Users (US-3.1 / spec US-3.1)
...
- `q`'s matched column(s) assumed to be `email`/`display_name` — not stated by the spec (db-design's own note, cross-referenced from `US-3.1-api-design.md` Open Question 3).
...
- Whether `PATCH /v1/admin/users/{id}`'s request body needs its own `reason` field (for `AdminAuditLog.reason`, OD-1) is unresolved by the API design — `US-3.1-openapi.yaml`'s `UpdateUserRequest` does not currently declare one.
