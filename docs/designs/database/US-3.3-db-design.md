# DB Design: View Audit Information (US-3.3 / spec US-3.3)

**Spec:** `docs/specifications/US-3.3-spec.md` (Pass with Issues, third run, 2026-09-02)
**API:** `docs/designs/api/US-3.3-openapi.yaml`, `US-3.3-api-design.md`
...
- `actor_id`, `actor_role`, `target_id`, `outcome`, `request_id`, `ip`, `user_agent`, `payload` — nullable, because not every event type populates every field (e.g. AU-AC9's own retention-job-execution record has no HTTP `request_id`/`ip`, and a system-initiated event has no `actor_role`). `payload` is the first `JSONB` column in this codebase (found during T2, IMPLEMENTATION stage — the only prior `postgresql.dialects` usage, `roles/models.py`, is `ARRAY`, not `JSONB`); flagged here since this design didn't call it out as a new pattern when written.
