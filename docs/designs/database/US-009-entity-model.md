# Entity Model: Multi-Factor Authentication / TOTP (US-2.5 / spec US-009)
...
No relationship changes on `UserRole`/`Role` — `granted_at` is a plain column read directly via the existing `user_roles` query path US-3.2 already established (`resolve_scopes_for_user` and its sibling role-name-lookup method, per `docs/specifications/US-009-mfa-totp-spec.md` FR-6's OD-7 resolution), not a new join.
