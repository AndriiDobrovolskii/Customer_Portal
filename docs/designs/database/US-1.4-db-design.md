# DB Design: US-1.4 Deactivate Account

**Spec:** `docs/specifications/US-1.4-spec.md` (Pass)
**API:** `docs/designs/api/US-1.4-openapi.yaml`, `US-1.4-api-design.md`
...
No `request_id`/`ip`/`user_agent`/`actor_role`/`target_id` columns: not required by any US-1.4 FR, and the existing two audit tables (`ProfileAuditLog`, email_verification's `AuditLog`) aren't uniform on these either — see Open Items.
...
3. **`account_lifecycle_audit_log` vs. `US-3.3`'s eventual unioned view** — whether the view needs `actor_role`/`request_id`/`target_id`/`ip`/`user_agent` uniformly across all five audit tables is a `US-3.3`-stage decision; this table matches its closest existing sibling rather than pre-guessing US-3.3's requirements.
