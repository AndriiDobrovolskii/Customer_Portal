...
**Contract:** `US-005-openapi.yaml`
**Spec:** `docs/specifications/US-005-login-spec.md` (Pass with Issues, accepted 2026-08-31 — `docs/reviews/specifications/US-005-spec-review.md`)
...
- Current `authenticate_user` checks only `email_verified`, never `UserStatus.DEACTIVATED` (`users.deactivated_at`/`status` both already exist on the model, per US-004) — FR-4's deactivated branch is unimplemented.
...
Refresh/rotation mechanics (US-2.3), session termination (US-2.2), MFA challenge branch (US-2.5), registration/email verification (US-1.1, US-1.2), RS256/JWKS signing (resolved OD-1), mobile JSON-body refresh transport (resolved OD-2) — unchanged from `docs/specifications/US-005-login-spec.md#out-of-scope`.
