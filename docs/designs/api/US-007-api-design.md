...
**Contract:** `US-007-openapi.yaml`
**Spec:** `docs/specifications/US-007-refresh-token-spec.md` (Pass with Issues, accepted 2026-09-01 — `docs/reviews/specifications/US-007-spec-review.md`, all 3 same-day findings resolved)
...
- **`429` is a new response shape for this endpoint, but not a new type slug.** Reuses `too-many-attempts` from login (US-2.1/US-005) rather than inventing a fresh one — consistent with the spec's own "no new error type slugs" statement, since the spec's claim was scoped to the `401` family and this is a distinct status the spec's Open Decision Resolutions section (OD-1) separately authorizes.
...
Initial token issuance (US-2.1), session listing (US-2.6), `X-Client-Type: mobile` body-delivery (resolved OD-2), linking `user_sessions` to `refresh_tokens.family_id` for reuse-detection to also kill a live access token (resolved OD-6) — unchanged from `docs/specifications/US-007-refresh-token-spec.md#out-of-scope`.
