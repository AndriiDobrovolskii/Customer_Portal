# API Design: Password Reset (US-2.4 / spec US-008)

**OpenAPI fragment:** `docs/designs/api/US-008-openapi.yaml`
...
- **NFR-005 (DB-enforced invariants):** atomic token consumption, same class of guarantee as `US-001`'s email uniqueness and `US-007`'s refresh-token consumption.
