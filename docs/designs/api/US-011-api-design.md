# API Design: Manage Users (US-3.1 / spec US-011)

**Source spec:** docs/specifications/US-011-manage-users-spec.md (revised 2026-09-02)
**Spec review:** docs/reviews/specifications/US-011-spec-review.md (Pass with Issues, accepted 2026-09-02)
**OpenAPI fragment:** docs/designs/api/US-011-openapi.yaml
...
9. ~~**`PATCH`'s request body has no `reason` field.**~~ **Resolved by `planner` 2026-09-02** (`US-011-implementation-plan.md` Architectural Change #4): `UpdateUserRequest` now requires `reason`, mirroring `deactivate`'s explicit `{reason}`.
