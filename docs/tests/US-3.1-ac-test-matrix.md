# Traceability Matrix: Manage Users (US-3.1 / spec US-3.1)

**Spec:** `docs/specifications/US-3.1-spec.md`
**Task breakdown:** `docs/plans/US-3.1-task-breakdown.md` (T7 unit, T8 integration — test code itself deferred to IMPLEMENTATION, matrix only at this stage, same sequencing as US-2.1–US-2.6/US-3.2)
**Written:** 2026-09-02
**Note (2026-09-02, post-RECONCILIATION):** this matrix was written before IMPLEMENTATION; several test function names below were renamed during actual coding (e.g. `test_get_admin_users_returns_200_and_paginates` shipped as `test_list_users_returns_200_and_paginates`). `docs/reviews/reconciliation/US-3.1-reconciliation.md`'s AC → Test table is the authoritative, verified name mapping — consult it, not this file, for the exact shipped test function per AC.

Every route under `/v1/admin/*` gets the full `AGENTS.md` §5 security case set (missing/invalid/expired token, insufficient permission scope) except `DELETE`, which has no permission check (any authenticated caller gets 405 per FR-17's resolved reading — see `US-3.1-api-design.md`).
...
| MU-AC17 / FR-17, resolved reading | Unauthenticated caller → 401, not 405 (MU-AC3's blanket rule still applies — see `US-3.1-api-design.md`'s resolution) | Integration | `test_delete_user_missing_token_returns_401_not_405` |
