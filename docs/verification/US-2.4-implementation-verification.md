# Verification Report: Password Reset (US-2.4 / spec US-2.4)
...
| All nested data eager-loaded | N/A | `PasswordResetToken` declares no `relationship()` (per `docs/designs/database/US-2.4-entity-model.md` and `app/modules/users/models.py`); the repository queries it directly by `token_hash`/`user_id`, never traverses from a loaded `User`. |
