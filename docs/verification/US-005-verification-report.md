...
**Story ID:** US-005 (spec's own field; backlog story is US-2.1)
...
`POST /v1/auth/login` is this story's one endpoint, and it is deliberately unauthenticated (`security: []` in the OpenAPI contract, `docs/designs/api/US-005-openapi.yaml`) — the four protected-route security cases don't apply to it by design, not by omission. This is the same treatment `US-004-verification-report.md`-equivalent reasoning would give any genuinely public endpoint.
