# Verification Report: Active Session Management (US-2.6 / spec US-2.6)
...
| All nested data eager-loaded | N/A | No `relationship()` was added — db-designer explicitly rejected a first-class `refresh_token_families` table (`docs/designs/database/US-2.6-db-design.md`); both new repository queries (`list_live_families_for_user`, `get_family_created_at_map_for_user`, `repository.py:340-386`) are raw `select()`/`DISTINCT ON`/`GROUP BY` statements, not ORM collection loads. |
...
| `extra="forbid"` + privilege exclusion on inbound schemas | N/A | This story adds no inbound schema — both endpoints have no request body (`docs/designs/api/US-2.6-openapi.yaml`); `SessionEntry`/`SessionListResponse` (`schemas.py`) are outbound-only. |
