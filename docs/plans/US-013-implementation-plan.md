# Implementation Plan: View Audit Information (US-3.3 / spec US-013)
...
**Impact analysis:** docs/impact-analysis/US-013-impact-analysis.md
...
3. **New compatibility view `audit_log_history`** — `UNION ALL` of `audit_log` with the four existing per-domain tables, each column-mapped and NULL-padded per `US-013-entity-model.md`'s mapping table. This is `AuditLogRepository.list_filtered()`'s actual query target, not `audit_log` alone.
...
| `app/modules/audit/models.py` | `AuditLog` per `US-013-entity-model.md` (daily-partitioned, `previous_hash`/`row_hash` trigger-computed, never application-settable). |
...
| `app/modules/audit/router.py` | `GET /v1/admin/audit-logs` per `US-013-openapi.yaml`, `response_model`/`status_code` declared (`AGENTS.md` §6.7). No `PATCH`/`PUT`/`DELETE` handler registered — Starlette's default `405` applies (verified against `app/main.py`'s actual exception handlers in impact-analysis; none intercept it). |
...
| `migrations/versions/<rev>_add_audit_log_and_history_view.py` | `audit_log` table + partitions + hash-chain trigger + `audit_log_history` view, per `US-013-db-design.md`. |
