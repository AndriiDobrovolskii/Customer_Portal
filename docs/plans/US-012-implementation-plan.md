# Implementation Plan: Manage Roles (US-3.2 / spec US-012)
...
**Impact analysis:** docs/impact-analysis/US-012-impact-analysis.md (including its 2026-09-01 user-resolved items)
...
| `app/modules/roles/models.py` | `Role`, `Permission`, `RolePermission`, `UserRole` per `US-012-entity-model.md`. |
...
| `app/modules/roles/router.py` | `GET /v1/admin/roles`, `PUT /v1/admin/users/{id}/roles` per `US-012-openapi.yaml`; `response_model`/`status_code` declared on both per `AGENTS.md` §6.7. |
