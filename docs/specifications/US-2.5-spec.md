...
- **Spec-review gap (grace-period clock has no data source), resolved 2026-09-01:** `user_roles` (US-3.2) gains a `granted_at` timestamp column (additive migration, nullable, backfilled to `now()` for existing rows). `PUT /v1/admin/users/{id}/roles` (US-3.2 FR-1) sets it on every full-replacement write that results in a role being held, so the 14-day grace-period clock in FR-6 has an unambiguous data source. This is a small, additive change to the already-merged US-3.2 schema, not a behavior change to US-3.2's endpoints.
...
**Derived from:** Not covered by any Acceptance Criterion in the source (the source's API Contract table documents the `204` response with no accompanying Gherkin); resolved per OD-6/OD-8, both flagged independently by `docs/reviews/specifications/US-2.5-spec-review.md`.
