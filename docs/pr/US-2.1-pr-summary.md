...
| implementation-verifier | Pass | `docs/verification/US-2.1-implementation-verification.md` |
| security-reviewer | Pass | `docs/reviews/security/US-2.1-security-review.md` |
| reconciliation-reviewer | Pass | `docs/reviews/reconciliation/US-2.1-reconciliation.md` (found + fixed one gap same-day: a missing test surfaced a real `verify_password_dummy()` logic bug, harmless in production since its return value was never used, now fixed) |

## ⚠️ Commit hygiene flags — read before staging

1. **`.gitignore` fix, deliberately included in this PR (user-approved 2026-08-31).** Eleven directories (`docs/{catalog,designs/api,designs/database,impact-analysis,plans,pr,reconciliation,reviews/plans,security,tests,verification}`) each carried a `*`/`!.gitignore` rule written when the directory was still empty — silently excluding every pipeline doc from git ever since, including all of US-1.4's own plan/design/verification/security/reconciliation docs (confirmed still untracked on disk today). A prior session's `US-1.4-pr-description.md` draft speculated this was "a deliberate repo convention" — re-investigated this session and concluded that reading doesn't hold up (the comment literally says "currently empty," and the whole point of this pipeline is a durable audit trail). User explicitly chose to fix it and include this story's own docs; US-1.4's equivalent docs are deliberately left untracked for now as a possible separate follow-up commit, not silently bundled here.
...
- Implements `POST /v1/auth/login` per `docs/specifications/US-2.1-spec.md` (spec review: Pass with Issues, accepted): credential verification, anti-enumeration timing parity via a dummy Argon2id path, brute-force throttling (10/account, 20/IP per 15 min, independent per-account reset), account-state gating, request validation, and audit logging — extending the pre-existing minimal endpoint (VE-AC5/VE-AC6 only) rather than replacing it.
- **Resolved OD-10 (found via `advisor()` review, not in the original spec text):** deactivated-account login within its 30-day grace period now reactivates the account instead of blocking it (DA-AC8), via a new cross-module `AccountService.reactivate_account()` call from `users/service.py` — the behavior US-1.4's own design doc had explicitly deferred to this story but the original spec never carried.
...
**Linked docs:** spec `docs/specifications/US-2.1-spec.md` · open decisions `docs/decisions/US-2.1-open-decisions.md` (10 resolved) · plan `docs/plans/US-2.1-implementation-plan.md` · task breakdown `docs/plans/US-2.1-task-breakdown.md`.
...
docs/designs/api/US-2.1-api-design.md                     (new)
docs/designs/api/US-2.1-openapi.yaml                      (new)
docs/designs/database/.gitignore                          (fixed)
docs/designs/database/US-2.1-db-design.md                 (new)
docs/designs/database/US-2.1-entity-model.md               (new)
docs/impact-analysis/.gitignore                           (fixed)
docs/impact-analysis/US-2.1-impact-analysis.md             (new)
docs/plans/.gitignore                                     (fixed)
docs/plans/US-2.1-implementation-plan.md                   (new)
docs/plans/US-2.1-task-breakdown.md                        (new)
docs/pr/.gitignore                                        (fixed)
docs/reconciliation/.gitignore                            (fixed)
docs/reviews/reconciliation/US-2.1-reconciliation.md        (new)
docs/reviews/plans/.gitignore                             (fixed)
docs/reviews/plans/US-2.1-plan-review.md                   (new)
docs/reviews/specifications/US-2.1-spec-review.md
docs/security/.gitignore                                  (fixed)
docs/reviews/security/US-2.1-security-review.md                    (new)
docs/specifications/US-2.1-spec.md
docs/tests/.gitignore                                     (fixed)
docs/tests/US-2.1-ac-test-matrix.md                   (new)
docs/verification/.gitignore                              (fixed)
docs/verification/US-2.1-implementation-verification.md            (new)
docs/workflow/active-story.yaml
docs/workflow/history.jsonl
docs/workflow/workflow-state.yaml
```

**Deliberately excluded:** `docs/catalog/US-1.4-pipeline-status.md`, `docs/designs/api/US-1.4-*`, `docs/designs/database/US-1.4-*`, `docs/impact-analysis/US-1.4-*`, `docs/plans/US-1.4-*`, `docs/pr/US-1.4-pr-summary.md`, `docs/reconciliation/US-1.4-*`, `docs/reviews/plans/US-1.4-*`, `docs/security/US-1.4-*`, `docs/tests/US-1.4-*`, `docs/verification/US-1.4-*` — all still-untracked from before this session, out of scope for this story's PR (see hygiene flag #1 above).
