# PR Draft: US-1.4 — Deactivate Account
...
| implementation-verifier | Pass | `docs/verification/US-1.4-implementation-verification.md` |
| security-reviewer | Pass | `docs/reviews/security/US-1.4-security-review.md` |
| reconciliation-reviewer | Pass | `docs/reviews/reconciliation/US-1.4-reconciliation.md` |

## ⚠️ Commit hygiene flag — read before staging

`git status` shows unrelated pending changes under `.claude/skills/` (new/modified skill definitions) that predate this story's work and are not part of US-1.4. **Do not include `.claude/` in this PR's commit** — stage only the files listed below. Bundling them would violate `AGENTS.md` §7.8 ("no drive-by... unrelated files").

## Suggested PR title

`feat: implement US-1.4 deactivate account (POST /v1/account/deactivate)`

## Summary

- Implements self-service account deactivation per `docs/specifications/US-1.4-spec.md` (spec review: Pass): `POST /v1/account/deactivate` verifies the caller's current password, atomically transitions the account to `deactivated` (guarding against a concurrent double-deactivation race), writes an `account_lifecycle_audit_log` entry, and sets a `revoke_before:{user_id}` Valkey key so every pre-existing access token is rejected on its next use.
- Introduces this project's first Valkey infrastructure: connection pool wired in `app.main`'s `lifespan`, `RevocationCache` gateway (`app/core/revocation_cache.py`) as a shared, fail-closed token-denylist primitive per `AGENTS.md` §3 — approved dependency addition `redis>=5.0` (user sign-off recorded in `docs/plans/US-1.4-implementation-plan.md`).
...
- FR-4–FR-10 (login/refresh/reactivation/purge-job/admin-deactivation behavior) are explicitly **not** implemented here — they're owned by US-2.1/US-2.2/US-2.3/US-3.1 and a future cron story; see `docs/designs/api/US-1.4-api-design.md`'s FR-ownership table.

**Linked docs:** spec `docs/specifications/US-1.4-spec.md` · plan `docs/plans/US-1.4-implementation-plan.md` · task breakdown `docs/plans/US-1.4-task-breakdown.md`.
...
- Full traceability: `docs/tests/US-1.4-ac-test-matrix.md` (AC → test function, reconciled against actual shipped test names).

## Risk / rollback

- Per `docs/plans/US-1.4-implementation-plan.md`'s Risks section: the `users/service.py` change touches the shared auth dependency every existing authenticated route depends on (`profile`) — mitigated by an explicit test proving an active user's token is unaffected (`revoke_before` absent).
