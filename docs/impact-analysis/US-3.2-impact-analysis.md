# Impact Analysis: Manage Roles (US-3.2 / spec US-3.2)
...
| `models.py` | models | `Role`, `Permission`, `RolePermission`, `UserRole` per `docs/designs/database/US-3.2-entity-model.md`. |
| `schemas.py` | schemas | `RoleRead`, `RoleCatalogueResponse`, `ReplaceUserRolesRequest`, `ReplaceUserRolesResponse` per `US-3.2-openapi.yaml`'s component schemas. |
...
| `router.py` | router | `GET /v1/admin/roles`, `PUT /v1/admin/users/{id}/roles` per `US-3.2-openapi.yaml`. |
