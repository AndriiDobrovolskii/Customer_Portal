# Impact Analysis: Manage Users (US-3.1 / spec US-3.1)

**Source spec:** docs/specifications/US-3.1-spec.md
**API design:** docs/designs/api/US-3.1-{openapi.yaml,api-design.md}
**DB design:** docs/designs/database/US-3.1-{db-design,entity-model.md}
...
| `schemas.py` | `UserRead`, `UserListResponse`, `CreateUserRequest`, `UpdateUserRequest`, `DeactivateUserRequest`, `ResendInviteResponse` — request/response shapes per `US-3.1-openapi.yaml`. |
...
| `router.py` | 7 routes (`GET`/`POST /v1/admin/users`, `GET`/`PATCH`/`POST .../deactivate`/`DELETE`/`POST .../resend-invite` on `/v1/admin/users/{id}`) matching `US-3.1-openapi.yaml` exactly. Imports `require_scope` from `app.modules.roles.dependencies` directly — see Cross-Module Ripple. |
...
**Yes, a migration is required.** Per `US-3.1-db-design.md`:
