# Impact Analysis: Multi-Factor Authentication / TOTP (US-2.5 / spec US-009)
...
| File | Reason |
|---|---|
| `models.py` | Add 4 columns to `User` (`mfa_enabled`, `mfa_secret_encrypted`, `mfa_activated_at`, `mfa_reenrollment_required`) and a new `MfaRecoveryCode` model, per `docs/designs/database/US-009-entity-model.md`. |
...
| `router.py` | 4 new routes: `POST /v1/auth/mfa/enroll`, `POST /v1/auth/mfa/activate`, `POST /v1/auth/mfa/verify`, `DELETE /v1/auth/mfa`, per `US-009-openapi.yaml`. |
