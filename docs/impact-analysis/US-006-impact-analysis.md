...
**Spec:** `docs/specifications/US-006-logout-spec.md`
**API design:** `docs/designs/api/US-006-openapi.yaml`, `US-006-api-design.md`
**DB design:** `docs/designs/database/US-006-db-design.md`, `US-006-entity-model.md`
...
- **No change.** Both endpoints (`POST /v1/auth/logout`, `POST /v1/auth/logout-all`) have no request body and a `204` response with no body (per `US-006-openapi.yaml`) — no new Pydantic schema class is needed.
...
- `get_authenticated_user(token, *, allow_revoked=False)` *(existing method, signature change)* — per `US-006-api-design.md`'s Open Question #1, add an opt-in parameter so `POST /v1/auth/logout`'s dependency can resolve a caller whose jti is already revoked (FR-4/resolved OD-2) without weakening every other authenticated route, which continues to call this method with the default `allow_revoked=False`.
