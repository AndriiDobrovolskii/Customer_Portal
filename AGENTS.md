...
**Structured multi-field validation (ARCHITECTURE.md §4.4.5).** A Pydantic field validator short-circuits on the first invalid field, so it can't return "email malformed **and** password weak" as one batched response. When a caller genuinely needs that (e.g. registration), keep those specific fields loosely typed on the schema (`str | None`, `SecretStr | None` — never bare) and do the joint validation in the service, collecting `FieldError`s into one domain exception. This is the one sanctioned exception to "schemas are the only inbound validation" — a password validated this way is still `SecretStr`, unwrapped via `.get_secret_value()` only where characters are actually checked.
...
**Exception ownership.** `core/exceptions.py` holds only the generic `DomainError` base (and any error truly shared by every module — none yet). A module-specific error (`EmailAlreadyRegisteredError`, `InvalidCredentialsError`, ...) lives in that module's own `exceptions.py`, never in `core`, which stays generic and domain-free (§3, top-level layers).
...
**RLS seeding.** A table with PostgreSQL Row-Level Security enabled (`FORCE ROW LEVEL SECURITY`) rejects a raw `INSERT`/`SELECT` through `db_session` (the non-superuser `app_runtime` role) unless the session's `app.actor_kind`/`app.actor_id` GUCs satisfy that table's policy — a direct `_seed_*()` helper writing a restricted row (e.g. `visibility='internal'`) MUST set the required context itself (`SET LOCAL` or `set_config(..., true)`) around its own insert and clear it afterward, never leaving it for the calling test to remember. A test asserting the RLS mechanism *itself* (a raw `SELECT` with the application-layer filter deliberately disabled) still manages `app.actor_kind` explicitly around that assertion — the seeder only covers its own write.
...
This is §1's "propose, never execute unilaterally" applied to the pipeline: the harness assembles evidence, a person decides when to act on it. `pr-preparer` drafts the content only; creating a local branch and commit for the story happens later, on the human's separate explicit instruction, never as part of `PR_PREPARATION` itself.

Pushing the branch and opening the Pull Request are performed by `pr-creator` (stage `PR_CREATION`) — the one stage skill in this workflow permitted to touch the remote (`git push`, then the `github` MCP server's `create_pull_request`/`update_pull_request`). It never runs on its own initiative: `story-orchestrator`'s `continue` mode treats reaching `PR_CREATION` as a stop condition exactly like `BACKLOG_SYNC`, and it only runs when a human gives it its own separate, explicit instruction (e.g. "create the PR for US-x.y") — approving `HUMAN_PR_APPROVAL` does not carry approval for this, and running it does not carry approval for `READY_FOR_PR`. This narrows the older blanket rule ("skills do not push, open, or merge Pull Requests") to one named exception, kept behind its own explicit trigger rather than folded into automatic advancement.
